...
[x,fs] = audioread(filename_mix01);
frames = get_audio_frames(x, win_size, hop_size);   
%[wf] = windowed_audio_frames(frames);
%intCQT_mix01 = get_cqt_spectrogram(wf, cqt_kernel);   
intCQT_mix01 = get_cqt_spectrogram(frames, cqt_kernel);   
%[intCQT_mix01, hop_size_samples] = computeCQT(filename_mix01);


%% computer the PLCA (multipitch detection)
[ww,pp,rr,xa] = plca3d(ww2, abs(intCQT_mix01), 27, k*4, 30);
